...
export default function CalendarCell({
  ...
  // ✅ Determine if the date belongs to the active month (only for MonthView)
  let inMonth = true;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-2">
        <div
          className={`text-sm font-medium ${
            today ? "text-indigo-600" : "text-slate-700"
          }`}
        >
          {format(date, "d")}
        </div>
            ...
            <div
              ...
              style={{
                backgroundColor: evt.color
                  ? `${evt.color}20` // transparent background
      ...
      {/* Today indicator */}
      {today && (
        <div className="absolute top-3 left-3 w-2 h-2 rounded-full bg-indigo-500" />
      )}
    </div>
  );
}
